...
/**
 * 修改Redisson中的BaseRemoteService的性能：
 * 1、优化responses的同步
 * 2、并发获取response 结果
 * 原BaseRemoteService获取response是串行的
 *
 * @author denglt
 */
...
//                                    Arrays.<Object> asList(ackName, responseQueueName, requestQueueName), 
    ...
    private <T extends RRemoteServiceResponse> RPromise<T> poll(final long timeout,
        ...
        synchronized (responses) {
            entry = responses.get(requestId.toString());
            if (entry == null) {
                entry = new ResponseEntry();
                ResponseEntry oldEntry = responses.putIfAbsent(requestId.toString(), entry);
                if (oldEntry != null) {
                    entry = oldEntry;
                }
            }
        ...
        responseFuture.addListener(new FutureListener<T>() {
            ...
            public void operationComplete(Future<T> future) throws Exception {
                if (future.isCancelled()) {
                    //synchronized (responses)
                    {
                        ResponseEntry entry = responses.get(requestId.toString());
                        if (entry == null) {
                            return;
                        }
                        synchronized (entry) {
                            ...
                            if (entry.getResponses().isEmpty()) {
                                responses.remove(requestId.toString(), entry);
                            }
                        }
                    }
                }
    ...
    private void pollTasks(final ResponseEntry entry) {
        ...
        future.addListener(new FutureListener<RRemoteServiceResponse>() {
            ...
            public void operationComplete(Future<RRemoteServiceResponse> future) throws Exception {
                ...
                //synchronized (responses)
                {
                    ResponseEntry entry = responses.get(response.getId());
                    if (entry == null) {
                        return;
                    }
                    synchronized (entry) {
                        ...
                        Result res;
                        if (response instanceof RemoteServiceAck) {
                            res = list.remove(0);
                        } else {
                        ...
                        if (entry.getResponses().isEmpty()) {
                            responses.remove(key.toString(), entry);
                        } else {
                            RBlockingQueue<RRemoteServiceResponse> responseQueue = redisson.getBlockingQueue(responseQueueName, codec);
                            responseQueue.takeAsync().addListener(this);
                        }
